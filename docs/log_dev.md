...
- 计划移除分享选项。
- 新增一个分支来重新设计数据读取的问题，考虑了一下APP本身的功能实现。除去视频播放的功能就没有其他的了，因为网络流媒体大多涉及到版权，在线内容这一块基本是死路了。
然后想到内容可以来自用户自己拍摄，但是我们要做的事情优势又在哪里呢？包括现在所开发的APP，这个功能跟手机本身的功能其实是冲突的。还做不到一个用户黏性高的功能。
那么这个应用的前景又在哪里？它的存在，是为了解决什么问题？