...
2016.12.29
- 关于遍历的性能优化，大多数的时候因为有些应用的目录层级比较深，导致遍历的时候浪费了太多的时间去做根本没有意义的遍历
想起以前工作中遇到的一个问题，由于文件目录层级较深，在打包的时候没有打包进去，查找问题发现是打包的时候只打包三个目录层级的
文件，一般的 APP 混存文件，特点就是目录层级较深。而用户往SD卡拷贝的视频，极少有超过三个目录层级的（这只是根据周围朋友的使用习惯判断，并没有做过调查）。
因此限制遍历的层级到第三层应该是比较科学的。
- 意见与反馈移动至设定菜单中
- 冷启动的黑白屏问题真的很烦人，之前在主页面启动前加了一个透明的activity，但是这样显得启动的时间要好久。还是需要尝试新的优化方案

2017.1.4
- firebase上出现了一个setDataSource的错误，没有更多的错误信息。查了一下可能是因为没有添加网络权限引起的。先改完迭代一个版本看下之后上传上来的情况
- 增加动态权限适配，兼容更高版本的api
- 修改设置目录的扫描层级选择
...
2017.5.22
- 经过一段时间的使用体验，文件搜索会请求外部存储设备的权限，并且搜索速度过慢，影响使用。Android 官方的文档中建议使用FileProvider来读取Android上的文件
故决定新增一个分支，留存原有的数据库及方法。寻求一个新的解决文件列表的方案。在此基础上文件访问层级的设定也变得不必要了。
- 移除分享选项。